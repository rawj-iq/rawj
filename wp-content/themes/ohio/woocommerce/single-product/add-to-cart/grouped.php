			...
			<?php
				...
				foreach ( $grouped_products as $grouped_product ) {
					...
					<tr id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
							...
							<label for="product-<?php echo esc_attr( $grouped_product->get_id() ); ?>">
								<?php
									if ( $grouped_product->is_visible() ) {
										echo '<a class="font-titles" href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>';
									} else {
						...
						<td class="price">
							<?php
								echo esc_attr( $grouped_product->get_price_html());
